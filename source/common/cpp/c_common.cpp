#include "rdno_core/c_config.h"
#include "rdno_core/c_timer.h"
#include "rdno_core/c_serial.h"
#include "rdno_core/c_str.h"
#include "rdno_core/c_system.h"
#include "rdno_core/c_app.h"

#include "common/c_network.secret.h"

namespace ncore
{
    namespace napp
    {
        s16 config_key_to_index(str_t const& str)
        {
            s16 param_id = -1;
            switch (str_len(str))
            {
                case 4: param_id = str_eq_n(str, "ssid", 4, false) ? nconfig::PARAM_ID_WIFI_SSID : -1; break;
                case 7: param_id = str_eq_n(str, "ap_ssid", 7, false) ? nconfig::PARAM_ID_AP_SSID : -1; break;
                case 8: param_id = str_eq_n(str, "password", 8, false) ? nconfig::PARAM_ID_WIFI_PASSWORD : -1; break;
                case 11:
                    if (str_eq_n(str, "ap_password", 11, false))
                        param_id = nconfig::PARAM_ID_AP_PASSWORD;
                    else if (str_eq_n(str, "remote_port", 11, false))
                        param_id = nconfig::PARAM_ID_REMOTE_PORT;
                    break;
                case 13: param_id = str_eq_n(str, "remote_server", 13, false) ? nconfig::PARAM_ID_REMOTE_SERVER : -1; break;
            }
            if (param_id < 0)
            {
                s32 value = 0;
                if (from_str(str, &value, 10) && value >= 0 && value < 256)
                {
                    param_id = static_cast<s16>(value);
                }
            }
            return param_id;
        }

        void config_init_default(nconfig::config_t* config)
        {
            nconfig::reset(config);  // Reset the configuration to default values
            const str_t ssid = str_const(WIFI_SSID);
            const str_t pass = str_const(WIFI_PASSWORD);
            nconfig::set_string(config, nconfig::PARAM_ID_WIFI_SSID, ssid);
            nconfig::set_string(config, nconfig::PARAM_ID_WIFI_PASSWORD, pass);
            char  ap_ssid_buffer[64];
            str_t ap_ssid = str_mutable(ap_ssid_buffer, sizeof(ap_ssid_buffer));
            str_append(ap_ssid, "HumanPresence-");
            nsystem::get_unique_id(ap_ssid);
            nconfig::set_string(config, nconfig::PARAM_ID_AP_SSID, ap_ssid);
            const str_t ap_pass = str_const("esp32768");
            nconfig::set_string(config, nconfig::PARAM_ID_AP_PASSWORD, ap_pass);
            const str_t remote_server = str_const(SERVER_IP);
            nconfig::set_string(config, nconfig::PARAM_ID_REMOTE_SERVER, remote_server);
            const s32 remote_port = SERVER_PORT;
            nconfig::set_int(config, nconfig::PARAM_ID_REMOTE_PORT, remote_port);
        }
    }  // namespace napp
}  // namespace ncore
